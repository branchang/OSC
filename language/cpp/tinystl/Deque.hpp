...
namespace TinySTL{
	...
	namespace Detail{
		...
		public:
			template<class U>
			friend dq_iter<U> operator + (const dq_iter<U>& it, typename dq_iter<U>::difference_type n);
			template<class U>
			friend dq_iter<U> operator + (typename dq_iter<U>::difference_type n, const dq_iter<U>& it);
			template<class U>
			friend dq_iter<U> operator - (const dq_iter<U>& it, typename dq_iter<U>::difference_type n);
			template<class U>
			friend dq_iter<U> operator - (typename dq_iter<U>::difference_type n, const dq_iter<U>& it);
			template<class U>
			friend typename dq_iter<T>::difference_type operator - (const dq_iter<U>& it1, const dq_iter<U>& it2);
			template<class U>
			friend void swap(dq_iter<U>& lhs, dq_iter<U>& rhs);
		};
	...
	public:
		...
		friend bool operator== (const deque<U, Alloc2>& lhs, const deque<U, Alloc2>& rhs);
		template <class U, class Alloc2>
		friend bool operator!= (const deque<U, Alloc2>& lhs, const deque<U, Alloc2>& rhs);
		template <class U, class Alloc2>
		friend void swap(deque<U, Alloc2>& x, deque<U, Alloc2>& y);
	};//end of deque
}
